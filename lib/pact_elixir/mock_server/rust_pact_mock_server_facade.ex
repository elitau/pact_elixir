defmodule PactElixir.RustPactMockServerFacade do
  ...
  def create_mock_server(_pact_json, _port), do: err()
  def mock_server_mismatches(_port), do: err()

  # @spec mock_server_matched(number) :: {:ok, boolean}
  def mock_server_matched(_port), do: err()
  def write_pact_file(_port, _dir_path), do: err()
  def cleanup_mock_server(_port), do: err()
